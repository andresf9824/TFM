...
for imagenes in datasets:
    ...
    llenar_huecos= fill_holes(morfologica_outline)
    
    #Llamamos a nuestra funcion de erosion
    
    erosion=erode(llenar_huecos)
    
    #Llamamos a nuestra funcion median
    
    mediana= median(erosion, 13)
    imagen_segmentada= mediana
    
    # Guardar la imagen umbralizada en la misma carpeta de destino con el mismo nombre
    
    IJ.saveAs(imagen_segmentada,"jpg", os.path.join(carpeta_destino, imagenes))
